/*
 * MIC_SD_RECORDER — הקלטה לכרטיס SD והשמעה
 * לוח : ESP32 WROOM 32D
 *
...
// אין הגבלת זמן — הקלטה נעצרת רק עם כפתור עצור או כשהSD מלא
...
bool hasRecording(int slot) {
    return SD.exists(recFile(slot).c_str());
}
...
void doRecord(int slot) {
    SD.remove(recFile(slot).c_str());
    File f = SD.open(recFile(slot).c_str(), FILE_WRITE);
    if (!f) {
    ...
    Serial.printf("[REC] חריץ %d — ללא הגבלת זמן\n", slot);
    ...
    snprintf(g_status, sizeof(g_status), "נשמר! חריץ %d", slot);
    g_state  = IDLE;
}

// ─── השמעה מקובץ ─────────────────────────────────────────────
void doPlay(int slot) {
    File f = SD.open(recFile(slot).c_str(), FILE_READ);
    if (!f) {
...
void setup() {
    Serial.begin(115200);
    Serial.println("\n==============================");
    Serial.println("  MIC SD RECORDER");
    Serial.println("==============================");
    ...
    if (!SD.begin(PIN_SD_CS, SPI, 4000000)) {
        Serial.println("  [!] SD נכשל — בדוק חיבורים");
    } else {
